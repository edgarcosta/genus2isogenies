// Eval-safe shared library for the isogeny-primes test drivers: the parser
// for the committed data file tests/corpus.txt (schema documented in its
// header), the field/curve constructors, and the entry classifiers. This
// file contains only declarations (no `load`, no `quit`, no AttachSpec), so
// it is safe to Read+eval as one concatenated unit under the CI run_tests.m
// contract as well as to `load` from the standalone drivers. The attach,
// engine guard, and corpus-load-or-quit that the loadable drivers need live
// in tests/_corpus.m, which loads this file.
