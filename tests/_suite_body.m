// Eval-safe body of the isogeny-primes test suite: the section procedures and
// the RunSuite dispatcher. This file contains only declarations (no `load`,
// no `quit`), so it can be Read+eval'd as one concatenated unit under the CI
// run_tests.m contract (Tests/isogenyprimes.m) as well as `load`ed from the
// standalone driver tests/test_isogenyprimes.m. It depends on the parser and
// constructors from tests/_lib.m being in scope, and on the engine spec being
// attached, both arranged by the caller. Failure is signalled by RunSuite
// returning false, never by quitting; the caller converts that to an exit
// code (standalone) or a raised error (CI).
...
// Dispatch every selected section and return whether all passed. Failure is
// signalled by the boolean return, never by quitting, so the caller controls
// the process exit (standalone) or raises through run_tests' aggregation (CI).
// The printed verdicts match the historical standalone output exactly: one
// "SECTION <name>: PASS" per passing section, "SECTION <name>: FAIL: ..." on
// failure, and a final "ALL SELECTED SECTIONS PASS" on overall success.
function RunSuite(corpus, section, cmscope)
    // Validate the section selector before dispatching: an unknown name would
    // match no section block below and, without this guard, run nothing yet
    // fall through to success. FAIL loudly instead.
    ...
    // Each section call is wrapped so a caught error flips ok rather than
    // aborting the run. The section procedures are called directly: `assigned`
    // reports false for a top-level procedure when queried from inside this
    // function (and also inside eval scope), so it cannot gate the calls; the
    // spec is attached before RunSuite runs (standalone: tests/_corpus.m quits
    // on a missing engine; CI: config.m plus the entry's AttachSpec), so the
    // procedures always bind.
    //
    ...
    // cmscope:=0, which gates the only selected section off) and must FAIL
    // rather than PASS; the corpus-driven procedures separately `error if
    // n eq 0` when a running section matches no entries, so a section that
    // runs but processes zero entries fails there.
    nRun := 0;
    ok := true;
    ...
    // section:=cm cmscope:=0, since unknown names are rejected up front) ran
    // zero tests: that is a vacuous pass, so FAIL it.
    if nRun eq 0 then
        ok := false;
